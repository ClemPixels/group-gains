    ...
    bot.onText(/\/assign (.+)/, async (msg, match) => {
        ...
        try {
            ...
            if ((msg.chat.type === "group" || msg.chat.type === "supergroup") &&
                ...
                if (isGroupExist) {
                    bot.sendMessage(groupId, `Group with Id: ${groupId} and Name: "${groupName}" is already assigned`);
                    return;
                }
                ...
                await userCollection.updateOne({ _id: new ObjectId(userId) }, // Find the user by ID
                {
                    $push: {
