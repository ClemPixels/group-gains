...
export const keyVerify = async () => {
    bot.onText(/\/key (.+)/, async (msg, match) => {
        ...
        if (!subscription) {
            bot.sendMessage(chatId, `The key you provided is incorrect or does not exist. Please check your key and try again. If the problem persists, contact support: egyayemi2006+support@gmail.com`);
            return;
        }
