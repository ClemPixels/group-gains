...
paypalCronRoute.get("/paypal", async (c) => {
    ...
    for (const pendingPayout of pendingPayouts) {
        const payoutItemDetail = await paypalV1AxiosInstance.get(`/payments/payouts-item/${pendingPayout.paypal.payout_item_id}`, {
            headers: {
                Authorization: `Bearer ${token}`,
            },
        });
        const payoutItemStatus = payoutItemDetail.data.transaction_status;
        await Payout.updateOne({ _id: pendingPayout._id }, { status: payoutItemStatus });
        if (payoutItemStatus === "SUCCESS") {
            await Wallet.updateOne({ owner: pendingPayout.owner }, {
                $inc: {
        ...
        io.to(pendingPayout.owner).emit("update-payout", "refetch");
        io.to(pendingPayout.owner).emit("update-wallet", "refetch");
    }
