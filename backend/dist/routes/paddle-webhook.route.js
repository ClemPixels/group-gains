        ...
        if (signature && rawRequestBody) {
            ...
            if (eventData.eventType) {
                if (eventData.eventType === "subscription.activated") {
                    ...
                    if (customer) {
                        customer = await Customer.findOneAndUpdate({ anonymous_key: anonymousKey }, {
                    ...
                    if (transaction) {
                        transaction = await Transaction.findOneAndUpdate({ anonymous_key: anonymousKey }, {
                            price: amount,
                            subscription: subscription._id,
                        }, { new: true });
                    }
                ...
                if (eventData.eventType === "transaction.paid") {
                    ...
                    if (transaction) {
                        await Transaction.updateOne({ anonymous_key: anonymousKey }, {
                            price: amount,
                            status: "paid",
                        });
                    }
                    ...
                    Promise.all([
                        await Group.updateOne({ owner: userId }, {
                            $inc: {
                                revenue: amount,
                            },
                        }),
                        await Wallet.updateOne({ owner: userId }, {
                            $inc: {
                                balance: amount,
                            },
                        }),
