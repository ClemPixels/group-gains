...
paypalRoute.post("/payout", async (c) => {
    ...
    const { paypal: { currency, email }, } = integration;
    ...
    const resPayoutCreate = await paypalV1AxiosInstance.post("/payments/payouts", {
        sender_batch_header: {
    ...
    const resPayoutDetails = await paypalV1AxiosInstance.get(`/payments/payouts/${payoutBatchId}?total_required=true`, {
        headers: {
            Authorization: `Bearer ${token}`,
        },
    });
    ...
    return c.json({ success: true, message: "Payout requested successful", result: payout }, 200);
});
paypalRoute.post("/connect", async (c) => {
    ...
    if (integration) {
        integration = await Integration.findOneAndUpdate({ owner: user.id }, {
            paypal: {
                email: email,
            },
        }, { new: true });
    ...
    return c.json({ success: true, message: "Paypal connected!", result: integration }, 201);
});
